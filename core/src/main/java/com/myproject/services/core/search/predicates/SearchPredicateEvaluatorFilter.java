package com.myproject.services.core.search.predicates;
...
public class SearchPredicateEvaluatorFilter extends AbstractPredicateEvaluator {
		...
		try {
			...
			if (node.hasProperty("jcr:primaryType")) {
				...
				if ((propertyValue.equals("cq:Page") || propertyValue.equals("cq:Tag")
						|| propertyValue.equals("dam:Asset")) && isAllowedPath(node)) {
					return true;
				} else {
					return false;
				}
	 ...
	 */
	private boolean isAllowedPath(Node node) throws RepositoryException {
	...
	private boolean isContainsPath(List<String> asList, String nodePath) {
		...
		while(iterator.hasNext()) {
			...
			log.debug("List Path: {} and Node Path: {}", next, nodePath);
			return nodePath.startsWith(next);
		}
		return false;
	}
